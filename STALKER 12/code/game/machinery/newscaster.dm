...
/obj/item/weapon/newspaper/attack_self(mob/user)
	if(ishuman(user))
		...
		switch(screen)
			if(0) //Cover
				dat+="<DIV ALIGN='center'><B><FONT SIZE=6>Hrifon</FONT></B></div>"
				dat+="<DIV ALIGN='center'><FONT SIZE=2>Napruzhenist mizh SRSR ta krayinamy-soyuznykamy vse shche zrostaye...</FONT></div><HR>"
				if(isemptylist(news_content))
					if(wantedAuthor)
						dat+="Contents:<BR><ul><B><FONT COLOR='red'>**</FONT>Important Security Announcement<FONT COLOR='red'>**</FONT></B> <FONT SIZE=2>\[page [pages+2]\]</FONT><BR></ul>"
					else
						dat+="<I>Other than the title, the rest of the newspaper is too damaged to read...</I>"
				else
					dat+="Contents:<BR><ul>"
